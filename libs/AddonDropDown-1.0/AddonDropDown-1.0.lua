...
local ADDT = LibStub:GetLibrary("AddonDropDownTemplates-1.0")
...
ADD.MAXBUTTONS = 8;
ADD.MAXLEVELS = 2;
ADD.BUTTON_HEIGHT = 16;
ADD.BORDER_HEIGHT = 15;
...
ADD.DEFAULT_TEXT_HEIGHT = nil;
-- List of open menus
ADD.OPEN_DROPDOWNMENUS = {};

local button = ADDT:CreateListTemplate(ADD, "ADD_DropDownList1", 1);
...
button = ADDT:CreateListTemplate(ADD, "ADD_DropDownList2", 2);
button:SetFrameStrata("FULLSCREEN_DIALOG");
button:SetToplevel(true);
button:SetSize(180, 10);

-- Functions
ADD.Delegate = CreateFrame("FRAME");

function ADD.Delegate:OnAttributeChanged (self, attribute, value)
	if ( attribute == "createframes" and value == true ) then
		ADD:CreateFrames(self:GetAttribute("createframes-level"), self:GetAttribute("createframes-index"));
...
ADD.Delegate:SetScript("OnAttributeChanged", function(...) ADD.Delegate:OnAttributeChanged (...) end);


function ADD:InitializeHelper (frame)
	-- This deals with the potentially tainted stuff!
	if ( frame ~= ADD.OPEN_MENU ) then
		ADD.MENU_LEVEL = 1;
	end

	-- Set the frame that's being intialized
	ADD.Delegate:SetAttribute("initmenu", frame);

	-- Hide all the buttons
	local button, dropDownList;
	for i = 1, ADD.MAXLEVELS, 1 do
		dropDownList = _G["ADD_DropDownList"..i];
		if ( i >= ADD.MENU_LEVEL or frame ~= ADD.OPEN_MENU ) then
			dropDownList.numButtons = 0;
			dropDownList.maxWidth = 0;
			for j=1, ADD.MAXBUTTONS, 1 do
				button = _G["ADD_DropDownList"..i.."Button"..j];
				button:Hide();
			end
			dropDownList:Hide();
		end
	end
	frame:SetHeight(ADD.BUTTON_HEIGHT * 2);
end
...
function ADD:Initialize(frame, initFunction, displayMode, level, menuList)
	frame.menuList = menuList;
	
	ADD:InitializeHelper(frame)

	-- Set the initialize function and call it.  The initFunction populates the dropdown list.
	if ( initFunction ) then
		ADD:SetInitializeFunction(frame, initFunction);
	...
	local dropDownList = _G["ADD_DropDownList"..level];
	dropDownList.dropdown = frame;
	dropDownList.shouldRefresh = true;

	ADD:SetDisplayMode(frame, displayMode);
end

function ADD:SetInitializeFunction(frame, initFunction)
	frame.initialize = initFunction;
end


function ADD:SetDisplayMode(frame, displayMode)
...
function ADD:RefreshDropDownSize(self)
	self.maxWidth = ADD:GetMaxButtonWidth(self);
	self:SetWidth(self.maxWidth + 25);

	for i=1, ADD.MAXBUTTONS, 1 do
...
function ADD:OnUpdate(self, elapsed)
	if ( self.shouldRefresh ) then
		ADD:RefreshDropDownSize(self);
...
function ADD:StartCounting(frame)
	if ( frame.parent ) then
		ADD:StartCounting(frame.parent);
	else
		frame.showTimer = ADD.SHOW_TIME;
		frame.isCounting = 1;
	end
end

-- Stop the countdown on a frame
function ADD:StopCounting(frame)
	if ( frame.parent ) then
		ADD:StopCounting(frame.parent);
	else
...
local ADD_SecureInfo = {};

local wipe = table.wipe;

function ADD:CreateInfo()
	-- Reuse the same table to prevent memory churn
	if ( issecure() ) then
		wipe(ADD_SecureInfo);
		return ADD_SecureInfo;
	else
		return wipe(ADD_ButtonInfo);
	end
end

function ADD:CreateFrames(level, index)

	while ( level > ADD.MAXLEVELS ) do
		ADD.MAXLEVELS = ADD.MAXLEVELS + 1;
		local newList = ADDT:CreateListTemplate(self, "ADD_DropDownList"..ADD.MAXLEVELS); --CreateFrame("Button", "DropDownList"..ADD.MAXLEVELS, nil, "ADD_DropDownListTemplate");
		newList:SetFrameStrata("FULLSCREEN_DIALOG");
		newList:SetToplevel(true);
		newList:Hide();
		newList:SetID(ADD.MAXLEVELS);
		newList:SetWidth(180)
		newList:SetHeight(10)
		for i=ADD.MINBUTTONS+1, ADD.MAXBUTTONS do
			local newButton = ADDT:CreateButtonTemplate(self, "ADD_DropDownList"..ADD.MAXLEVELS.."Button"..i, newList); -- CreateFrame("Button", "DropDownList"..ADD.MAXLEVELS.."Button"..i, newList, "ADD_DropDownMenuButtonTemplate");
			newButton:SetID(i);
		end
	end

	while ( index > ADD.MAXBUTTONS ) do
		ADD.MAXBUTTONS = ADD.MAXBUTTONS + 1;
		for i=1, ADD.MAXLEVELS do
			local newButton = ADDT:CreateButtonTemplate(self, "ADD_DropDownList"..i.."Button"..ADD.MAXBUTTONS, _G["ADD_DropDownList"..i]); --  CreateFrame("Button", "ADD_DropDownList"..i.."Button"..ADD.MAXBUTTONS, _G["ADD_DropDownList"..i], "ADD_DropDownMenuButtonTemplate");
			newButton:SetID(ADD.MAXBUTTONS);
		end
	end
end
...
function ADD:AddSeparator(level)
	...
	ADD:AddButton(separatorInfo, level);
end

function ADD:AddButton(info, level)
	--[[
	...
	ADD.Delegate:SetAttribute("createframes-level", level);
	ADD.Delegate:SetAttribute("createframes-index", index);
	ADD.Delegate:SetAttribute("createframes", true);

	listFrame = listFrame or _G["ADD_DropDownList"..level];
	...
	if (info.iconOnly and info.icon) then
		...
		ADD:SetIconImage(icon, info.icon, info.iconInfo);
		icon:ClearAllPoints();
		icon:SetPoint("LEFT");
	end
	...
	local xPos = 5;
	local yPos = -((button:GetID() - 1) * ADD.BUTTON_HEIGHT) - ADD.BORDER_HEIGHT;
	...
	if ( frame ) then
		if ( ADD:GetSelectedName(frame) ) then
			if ( button:GetText() == ADD:GetSelectedName(frame) ) then
				info.checked = 1;
			end
		elseif ( ADD:GetSelectedID(frame) ) then
			if ( button:GetID() == ADD:GetSelectedID(frame) ) then
				info.checked = 1;
			end
		elseif ( ADD:GetSelectedValue(frame) ) then
			if ( button.value == ADD:GetSelectedValue(frame) ) then
				info.checked = 1;
			end
		end
	end
	...
	if ( info.hasColorSwatch ) then
		_G["ADD_DropDownList"..level.."Button"..index.."ColorSwatch".."NormalTexture"]:SetVertexColor(info.r, info.g, info.b);
		button.r = info.r;
		button.g = info.g;
		button.b = info.b;
		colorSwatch:Show();
	...
	ADD:CheckAddCustomFrame(listFrame, button, info);

	button:SetShown(button.customFrame == nil);

	button.minWidth = info.minWidth;

	width = max(ADD:GetButtonWidth(button), info.minWidth or 0);
	...
	listFrame:SetHeight((index * ADD.BUTTON_HEIGHT) + (ADD.BORDER_HEIGHT * 2));
end

function ADD:CheckAddCustomFrame(self, button, info)
	...
	if customFrame then
		...
		ADD:RegisterCustomFrame(self, customFrame);
	end
end

function ADD:RegisterCustomFrame(self, customFrame)
	self.customFrames = self.customFrames or {}
	table.insert(self.customFrames, customFrame);
end

function ADD:GetMaxButtonWidth(self)
	local maxWidth = 0;
	for i=1, self.numButtons do
		local button = _G[self:GetName().."Button"..i];
		local width = ADD:GetButtonWidth(button);
...
function ADD:GetButtonWidth(button)
...
function ADD:Refresh(frame, useValue, dropdownLevel)
	...
	if ( not dropdownLevel ) then
		dropdownLevel = ADD.MENU_LEVEL;
	end

	local listFrame = _G["ADD_DropDownList"..dropdownLevel];
	listFrame.numButtons = listFrame.numButtons or 0;
	-- Just redraws the existing menu
	for i=1, ADD.MAXBUTTONS do
		button = _G["ADD_DropDownList"..dropdownLevel.."Button"..i];
		checked = nil;
		...
		if not button.notCheckable and button:IsShown() then
			...
			checkImage = _G["ADD_DropDownList"..dropdownLevel.."Button"..i.."Check"];
			uncheckImage = _G["ADD_DropDownList"..dropdownLevel.."Button"..i.."UnCheck"];
			if ( checked ) then
				somethingChecked = true;
				local icon = GetChild(frame, frame:GetName(), "Icon");
				if (button.iconOnly and icon and button.icon) then
					ADD:SetIconImage(icon, button.icon, button.iconInfo);
				elseif ( useValue ) then
					ADD:SetText(frame, button.value);
					icon:Hide();
				else
					ADD:SetText(frame, button:GetText());
					icon:Hide();
				end
				button:LockHighlight();
				checkImage:Show();
	...
	if(somethingChecked == nil) then
		ADD:SetText(frame, VIDEO_QUALITY_LABEL6);
	end
	if (not frame.noResize) then
		...
		ADD:RefreshDropDownSize(_G["ADD_DropDownList"..dropdownLevel]);
	end
end

function ADD:RefreshAll(frame, useValue)
	for dropdownLevel = ADD.MENU_LEVEL, 2, -1 do
		local listFrame = _G["ADD_DropDownList"..dropdownLevel];
		if ( listFrame:IsShown() ) then
			ADD:Refresh(frame, nil, dropdownLevel);
		end
	end
	-- useValue is the text on the dropdown, only needs to be set once
	ADD:Refresh(frame, useValue, 1);
end

function ADD:SetIconImage(icon, texture, info)
...
function ADD:SetSelectedName(frame, name, useValue)
	frame.selectedName = name;
	frame.selectedID = nil;
	frame.selectedValue = nil;
	ADD:Refresh(frame, useValue);
end

function ADD:SetSelectedValue(frame, value, useValue)
...
function ADD:SetSelectedID(frame, id, useValue)
	frame.selectedID = id;
	frame.selectedName = nil;
	frame.selectedValue = nil;
	ADD:Refresh(frame, useValue);
...
function ADD:GetSelectedName(frame)
	return frame.selectedName;
end

function ADD:GetSelectedID(frame)
	if ( frame.selectedID ) then
		...
		local button;
		for i=1, ADD.MAXBUTTONS do
			button = _G["ADD_DropDownList"..ADD.MENU_LEVEL.."Button"..i];
			-- See if checked or not
			if ( ADD:GetSelectedName(frame) ) then
				if ( button:GetText() == ADD:GetSelectedName(frame) ) then
					return i;
...
function ADD:GetSelectedValue(frame)
	return frame.selectedValue;
end

function ADD:OnClick(self)
...
function ADD:HideDropDownMenu(level)
	local listFrame = _G["ADD_DropDownList"..level];
	listFrame:Hide();
end

function ADD:ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	if ( not level ) then
		level = 1;
	end
	ADD.Delegate:SetAttribute("createframes-level", level);
	ADD.Delegate:SetAttribute("createframes-index", 0);
	ADD.Delegate:SetAttribute("createframes", true);
	ADD.MENU_LEVEL = level;
	ADD.MENU_VALUE = value;
	local listFrame = _G["ADD_DropDownList"..level];
	local listFrameName = "ADD_DropDownList"..level;
	local tempFrame;
	...
	if ( listFrame:IsShown() and (ADD.OPEN_MENU == tempFrame) ) then
		listFrame:Hide();
	else
		...
		if ( level == 1 ) then
			ADD.Delegate:SetAttribute("openmenu", dropDownFrame);
			...
			if ( not anchorName ) then
				...
				if ( dropDownFrame.relativeTo ) then
					...
					relativeTo = GetChild(ADD.OPEN_MENU, ADD.OPEN_MENU:GetName(), "Left");
				end
			...
			if ( not dropDownFrame ) then
				dropDownFrame = ADD.OPEN_MENU;
			end
			listFrame:ClearAllPoints();
			-- If this is a dropdown button, not the arrow anchor it to itself
			if ( strsub(button:GetParent():GetName(), 0,12) == "ADD_DropDownList" and strlen(button:GetParent():GetName()) == 13 ) then
				anchorFrame = button;
			else
		...
		ADD:Initialize(dropDownFrame, dropDownFrame.initialize, nil, level, menuList);
		...
		if ( level == 1 ) then
			...
			listFrame:ClearAllPoints();
			listFrame.parentLevel = tonumber(strmatch(anchorFrame:GetName(), "ADD_DropDownList(%d+)"));
...
function ADD:CloseDropDownMenus(level)
	if ( not level ) then
		level = 1;
	end
	for i=level, ADD.MAXLEVELS do
		_G["ADD_DropDownList"..i]:Hide();
	end
end

function ADD:OnHide(self)
	local id = self:GetID()
	if ( self.onHide ) then
		self.onHide(id+1);
		self.onHide = nil;
	end
	ADD:CloseDropDownMenus(id+1);
	ADD.OPEN_DROPDOWNMENUS[id] = nil;
	if (id == 1) then
		ADD.OPEN_MENU = nil;
	end
...
function ADD:SetWidth(frame, width, padding)
...
function ADD:ClearAll(frame)
	...
	for i=1, ADD.MAXBUTTONS do
		...
		checkImage = _G["ADD_DropDownList"..ADD.MENU_LEVEL.."Button"..i.."Check"];
		checkImage:Hide();
		uncheckImage = _G["ADD_DropDownList"..ADD.MENU_LEVEL.."Button"..i.."UnCheck"];
		uncheckImage:Hide();
	end
end
...
function ADD:JustifyText(frame, justification)
	local frameName = frame:GetName();
...
function ADD:SetAnchor(dropdown, xOffset, yOffset, point, relativeTo, relativePoint)
...
function ADD:GetCurrentDropDown()
	if ( ADD.OPEN_MENU ) then
		return ADD.OPEN_MENU;
	elseif ( ADD.INIT_MENU ) then
		return ADD.INIT_MENU;
	end
end

function ADD:GetChecked(self)
	return _G[self:GetName().."Check"]:IsShown();
end

function ADD:GetName(self)
	return _G[self:GetName().."NormalText"]:GetText();
end

function ADD:OpenColorPicker(self, button)
	CloseMenus();
	if ( not button ) then
		button = self;
	end
	ADD.MENU_VALUE = button.value;
	OpenColorPicker(button);
end

function ADD:DisableButton(level, id)
	_G["ADD_DropDownList"..level.."Button"..id]:Disable();
	_G["ADD_DropDownList"..level.."Button"..id.."Check"]:SetDesaturated(true);
	_G["ADD_DropDownList"..level.."Button"..id.."Check"]:SetAlpha(0.5);
	_G["ADD_DropDownList"..level.."Button"..id.."UnCheck"]:SetDesaturated(true);
	_G["ADD_DropDownList"..level.."Button"..id.."UnCheck"]:SetAlpha(0.5);
end

function ADD:EnableButton(level, id)
	local button = _G["ADD_DropDownList"..level.."Button"..id];
	button:Enable();
	
	_G["ADD_DropDownList"..level.."Button"..id.."Check"]:SetDesaturated(false);
	_G["ADD_DropDownList"..level.."Button"..id.."Check"]:SetAlpha(1);
	_G["ADD_DropDownList"..level.."Button"..id.."UnCheck"]:SetDesaturated(false);
	_G["ADD_DropDownList"..level.."Button"..id.."UnCheck"]:SetAlpha(1);
end

function ADD:SetButtonText(level, id, text, colorCode)
	local button = _G["ADD_DropDownList"..level.."Button"..id];
	if ( colorCode) then
...
function ADD:SetButtonNotClickable(level, id)
	_G["ADD_DropDownList"..level.."Button"..id]:SetDisabledFontObject(GameFontHighlightSmallLeft);
end

function ADD:SetButtonClickable(level, id)
	_G["ADD_DropDownList"..level.."Button"..id]:SetDisabledFontObject(GameFontDisableSmallLeft);
end

function ADD:DisableDropDown(dropDown)
...
function ADD:EnableDropDown(dropDown)
	local dropDownName = dropDown:GetName();
	local label = GetChild(dropDown, dropDownName, "Label");
	if ( label ) then
...
function ADD:IsEnabled(dropDown)
	return not dropDown.isDisabled;
end

function ADD:GetValue(id)
	--Only works if the dropdown has just been initialized, lame, I know =(
	local button = _G["ADD_DropDownList1Button"..id];
	if ( button ) then
		return _G["ADD_DropDownList1Button"..id].value;
	else
		return nil;
	end
end

-- function ADD:OpenColorPicker(info)
...
-- UIDropDownMenu.xml converted to lua

function ADD:CreateMenuTemplate(name, parent, id, frameType)
	return ADDT:CreateMenuTemplate(self, name, parent, id, frameType);
end
