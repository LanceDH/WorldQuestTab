	...
	table.sort(tbl, function(a, b)
				if (filterId == _V["FILTER_TYPES"].faction) then
					...
					if(type(a) == "number" and type(b) == "number")then
						local nameA = C_Reputation.GetFactionDataByID(tonumber(a));
						local nameB = C_Reputation.GetFactionDataByID(tonumber(b));
...
local function ConvertOldSettings(version)
	...
	if (version < "8.0.1") then
		...
		WQT.db.global.filters = WQT.db.global.filters or {}
		WQT.db.global.filters[1] = WQT.db.global.filters[1] or { flags = {} }
...
function WQT_ListButtonMixin:FactionOnEnter(frame)
    ...
    if (factionId) then
        local factionInfo = WQT_Utils:GetFactionDataInternal(factionId)
        if factionInfo and factionInfo.name then  -- Ensure factionInfo and its name are valid
            GameTooltip:SetOwner(frame, "ANCHOR_RIGHT", -5, -10);
            --GameTooltip:SetText(factionInfo.name, nil, nil, nil, nil, true);
        else
            GameTooltip:SetOwner(frame, "ANCHOR_RIGHT", -5, -10);
            GameTooltip:SetText("Unknown Faction", nil, nil, nil, nil, true);  -- Fallback text if faction info is missing
        end
    end
end
...
function WQT_ScrollListMixin:DisplayQuestList()
	...
	local shouldShowZone = WQT.settings and WQT.settings.list and WQT.settings.list.showZone and
                      (WQT.settings.list.alwaysAllQuests or (mapInfo and (mapInfo.mapType == Enum.UIMapType.Continent or mapInfo.mapType == Enum.UIMapType.World)));
...
function WQT_QuestCounterMixin:UpdateVisibility()
	local shouldShow = WQT.settings and WQT.settings.general and WQT.settings.general.questCounter and QuestScrollFrame:IsShown();
...
function WQT_CoreMixin:OnLoad()
	...
	if bountyBoard and bountyBoard.OnTabClick then
	...
	if bountyBoard and bountyBoard.RefreshSelectedBounty then
	...
	if bountyBoard and bountyBoard.AnchorBountyTab then
	...
	-- Check if Background exists before trying to set its position
